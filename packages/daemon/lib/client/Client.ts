...
export class Client {
  ...
  public async setIrcManager(): Promise<void> {
    ...
    this.ircManager.on('orderoffermessage', async (from, to, msg) => {
      const orderOffer = OrderOfferV0.deserialize(Buffer.from(msg, 'hex'));
      await this.db.order.saveOrderOfferByNick(orderOffer, from);
      // TODO: websocket send
    });

    this.ircManager.on('orderacceptmessage', async (from, to, msg) => {
      const orderAccept = OrderAcceptV0.deserialize(Buffer.from(msg, 'hex'));
      await this.db.order.saveOrderAcceptByNick(orderAccept, from);
      // TODO: websocket send
    });
