...
export class BufferWriter {
   ...
   */
  public writeVarInt(val: bigint | number): void {
   ...
   *   0xff = 8 byte number (9 bytes total)
   */
  public writeBigSize(val: bigint | number): void {
    const num = BigInt(val);
    if (num < BigInt(0xfd)) {
      this.writeUInt8(Number(num));
    } else if (num < BigInt(0x10000)) {
   ...
   * @param val
   */
  public writeTUInt16(val: number): void {
    if (val === 0) return;
   ...
   */
  public writeTUInt32(val: number): void {
    if (val === 0) return;
   ...
   */
  public writeTUInt64(val: bigint): void {
   ...
   * @param needed
   */
  private _expand(needed: number): void {
   ...
   */
  private _writeStandard(fn: string, val: number, len: number): void {
    this._expand(len);
